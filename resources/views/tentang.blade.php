...
@endsection
 ...
 <div class="w-full md:w-3/4 mx-auto mt-5 p-4 shadow-lg rounded-lg bg-white flex flex-col">
    <!-- Text section - always on top -->
    <div class="flex flex-col mb-4 md:mb-0 md:mr-8">
      <p class="text-xl md:text-3xl font-black leading-tight font-sans text-[#5e6f52]">
        Dukungan Kami terhadap SDGs
      </p>
      <p class="mt-2 text-gray-700 text-sm md:text-base leading-relaxed font-sans">
        Kami percaya bahwa masa depan yang bersih dan berkelanjutan<br class="hidden md:block">
        dimulai dari <span class="font-semibold text-[#5e6f52]">aksi nyata</span> dan <span class="font-semibold text-[#5e6f52]">teknologi yang tepat</span>.
      </p>
    </div>
    
    <!-- Icons section - single line on mobile -->
    <div class="flex flex-nowrap overflow-x-auto justify-start md:justify-center gap-2 md:gap-4 py-2">
      <img src="{{ asset('build/images/sdg11.png') }}" alt="" class="flex-none w-16 h-16 md:w-20 md:h-20 rounded-lg hover:scale-110 hover:shadow-lg transition-transform duration-300">
      <img src="{{ asset('build/images/sdg12.png') }}" alt="" class="flex-none w-16 h-16 md:w-20 md:h-20 rounded-lg hover:scale-110 hover:shadow-lg transition-transform duration-300">
      <img src="{{ asset('build/images/sdg13.png') }}" alt="" class="flex-none w-16 h-16 md:w-20 md:h-20 rounded-lg hover:scale-110 hover:shadow-lg transition-transform duration-300">
      <img src="{{ asset('build/images/sdg17.png') }}" alt="" class="flex-none w-16 h-16 md:w-20 md:h-20 rounded-lg hover:scale-110 hover:shadow-lg transition-transform duration-300">
    </div>
</div>

</div>

<div class="mt-6 w-full md:w-3/4 mx-auto">
    @php
    $profiles = [
      ['name' => 'Arief Huda','role' => 'Full Stack Developer','desc' => 'Bolivia, Bocah Liar Helvetia','img'  => asset('build/images/ariefganteng.png'),],
      ['name' => 'Markocop','role' => 'Full Stack Developer','desc' => 'Kembaran Arief dari Multiverse','img'  => asset('build/images/ariefganteng.png'),],
    ...
    <div class="md:hidden relative w-full overflow-hidden bg-white rounded-lg flex flex-nowrap overflow-x-auto scrollbar-hide snap-x snap-mandatory" style="scroll-behavior: smooth;">
      @foreach ($profiles as $index => $profile)
        <div class="inline-block flex-shrink-0 w-full snap-center text-center px-4">
          <p class="text-xl font-bold text-gray-700 mb-3 mt-5">Binary Buddies</p>
          <img src="{{ $profile['img'] }}" alt="Foto {{ $profile['name'] }}" class="h-60 mx-auto drop-shadow-lg" style="filter: drop-shadow(0 4px 6px rgba(204,193,78,1));">
          <p class="text-lg font-semibold text-gray-800">{{ $profile['name'] }}</p>
          <p class="text-sm text-gray-600">{{ $profile['role'] }}</p>
          <p class="mt-3 text-sm italic text-gray-500 max-w-xs mx-auto">{{ $profile['desc'] }}</p>
        </div>
    ...
    <div class="hidden md:flex rounded-lg bg-white overflow-visible">
      <div class="relative w-full overflow-hidden bg-white rounded-lg flex">
        @foreach ($profiles as $index => $profile)
          <div class="flex-shrink-0 w-full text-center px-4">
            <p class="text-xl font-bold text-gray-700 mb-3 mt-5">Binary Buddies</p>
            <img src="{{ $profile['img'] }}" alt="Foto {{ $profile['name'] }}" class="h-100 ml-18 drop-shadow-lg -mt-16 -mb-25" style="filter: drop-shadow(0 4px 6px rgba(204,193,78,1));">
            <p class="text-lg font-semibold text-gray-800">{{ $profile['name'] }}</p>
            <p class="text-sm text-gray-600">{{ $profile['role'] }}</p>
        ...
        <div class="flex overflow-visible">
          <div class="flex">
            <div class="flex mt-5 flex-col">
              <article class="ml-9 w-80 h-45 bg-gradient-to-br from-white to-green-800 transition-transform duration-300 ease-in-out hover:scale-105 hover:drop-shadow-lg hover:drop-shadow-cyan-500/50 hover:z-10"
                       style="clip-path: polygon(0% 0, 50% 0, 80% 100%, 0% 100%)">
                <img src="{{ asset('build/images/bill.png') }}" alt="" class="h-80 -mt-27 -ml-5">
              </article>
              <article class="ml-9 w-166 mt-5 h-40 bg-green-500 transition-transform duration-300 ease-in-out hover:scale-105 drop-shadow-lg drop-shadow-cyan-500/50 hover:z-10"
                       style="clip-path: polygon(0% 0, 40% 0, 53% 100%, 0% 100%)">
                <img src="{{ asset('build/images/ipeh.png') }}" alt="" class="h-80 -mt-30 ml-10">
              </article>
            </div>
            <div class="flex flex-col">
              <div class="flex">
                ...
                <article class="-ml-50 h-45 w-81 bg-red-100 mt-5 transition-transform duration-300 ease-in-out hover:scale-105 hover:drop-shadow-lg hover:drop-shadow-cyan-500/50 hover:z-10"
                         style="clip-path: polygon(30% 0, 100% 0, 100% 100%, 0% 100%)">
                  <img src="{{ asset('build/images/nahya.png') }}" alt="" class="h-80 -mt-30 ml-20">
                </article>
              </div>
              <article class="-ml-94 mt-5 mb-5 w-85 h-40 bg-gradient-to-tl from-white to-green-900 transition-transform duration-300 ease-in-out hover:scale-105 hover:drop-shadow-lg hover:drop-shadow-cyan-500/50 hover:z-10"
                       style="clip-path: polygon(0% 0, 100% 0, 100% 100%, 26% 100%)">
                <img src="{{ asset('build/images/ariefganteng2.png') }}" alt="" class="h-70 -mt-20 ml-25">
              </article>
            </div>
          </div>
          <div class="h-100 w-30 bg-[#ccc14e] rounded-r-lg ml-auto text-white flex items-center justify-center"></div>
